...
discriminants in `packages/core/src/actions.ts` are the source of truth**; 11 ops on
`main` (the `feat/copilot-intelligence` work merged via PR #18, 2026-07-06):
...
T7 is done: every store command routes through `applyActions`
...
locally, with no account and no server. 370 tests green as of 2026-07-06
...
| T20 | ✅ **Done.** Both wholesale-swap gaps are now guarded: `acceptDraft` re-validates the draft with `SchemaSchema.safeParse` before the swap (invalid drafts are discarded, surfaced as an error chat message, AND reported inline at whichever Accept button was clicked — `acceptDraft` returns an `AcceptDraftResult` the canvas draft bar and Suggestions tab render locally; the draft is _built_ via `applyActions` in `CopilotPanel.tsx`, so the parse is a contract guard, not a replay); and `loadProject`'s activate path validates schema + sources + chat via `validateProjectRecord`, which shares one validation core (`parseProjectContents` in `serialize.ts`) with the file-import path so the two contracts can't drift — a corrupted IndexedDB record is rejected with a surfaced error, never loaded, and a corrupt last-active record clears the persisted active pointer at bootstrap instead of re-firing every launch. Undo/redo restore remains the one sanctioned unvalidated swap (inherent to snapshot history). | `apps/web/src/store/schemaStore.ts`, `apps/web/src/persistence/{serialize,useProjects}.ts` |
...
| #   | Task                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | Unblocks                                                  | Where                                                                                                                                                               |
| --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| T9  | ✅ **Done.** Model side: `rowCount` on `SourceSchema` (`parse/types.ts`), captured uncapped at parse; `sampleRows`/`distinctValues`/`stats` also exist. UI side: per-source row counts in the sources-panel card meta, and an optional summed `rowCount` on `ProjectSummary` shown in the Home card footer — present only when EVERY source has a known count, so projects with legacy (pre-capture) sources omit the total rather than showing a confident undercount.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         | DESIGN #7 (source row counts), #15 (project footer rows). | UI: sources panel + `apps/web/src/persistence/projectStore.ts`                                                                                                      |
...
| T12 | ✅ **Done.** Live credential validation on save: `validateCredentialLive` (`ai/validateCredential.ts`) pings the provider's models endpoint (all three providers, via `listModels`) and distinguishes a rejected key (401/403) from an unreachable server — local failures are phrased as server/CORS problems, not bad credentials. `ByoKeyPage` gates Save behind format check → live check with a `validating` state (disabled inputs, "Checking…" button, helper-line status) and the existing inline-error surface; an **unreachable** provider (offline, 429/500) doesn't lock the user out — the next click is an explicit "Save anyway" that stores the credential unverified, while rejected (401/403) keys stay blocked. Last-4 display already existed (`maskKey` in `SettingsPage`); non-remembered keys show from the in-memory context with the "In memory · this session only" caption.                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| -------- | --------------------------------------------- | --------------- | ------------------------------------------------------------------ |
| 1        | Real provider key validation                  | T12             | ✅ done                                                            |
| 2        | `keyStatus: validating` state                 | T12             | ✅ done                                                            |
| 3        | OpenAI + Local providers                      | T18/T14         | ✅ both shipped (Local JSON-fallback for non-tool models deferred) |
| 4        | Settings page + key revocation                | —               | ✅ done                                                            |
| 5        | Last-4 display after save                     | T12             | ✅ done (maskKey in Settings; in-memory keys covered)              |
| 6        | Proposed/applied relationship state on canvas | T10             | deferred                                                           |
| 7        | Source-file row counts                        | T9              | ✅ done                                                            |
...
| 15       | Project row counts in card footer             | T9 (root cause) | ✅ done                                                            |
...
- **Don't both edit the store/canvas spine in the same window.** Suggested split:
  ...
  - **Owner B → model track:** T10 (the `Relationship.status` model change) plus the UI
    wiring T9 left open (DESIGN #7/#15). (T7, T9, and T20 are done.)
...
- **Branch base:** `feat/copilot-intelligence` is merged (PR #18) — §3's contract now
  describes `main`, so base both the model track and Owner A's packaging work (T5/T6)
  directly on `main`.
...
- **R5 (fold-validation, 2026-07-03): CLEAN.** All R4-introduced claims confirmed against
  ...
  clean.**
- **M1 (merge doc-sync, 2026-07-06):** `feat/copilot-intelligence` merged to `main` via
  PR #18 (`main` now carries the 11-op contract + multi-provider surface). Folded: dropped
  the §3 branch caveat (the 10-op/unmerged delta is gone); §4 rows for the copilot overhaul,
  multi-provider AI, and T18 re-marked merged; the "branch base" note now points model-track
  work at `main`; test count trued up to 370 (`pnpm test`, 2026-07-06). CLAUDE.md's op line
  was already correct at 11 — left as-is.
